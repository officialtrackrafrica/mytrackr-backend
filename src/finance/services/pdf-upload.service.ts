...
export class PdfUploadService {
  ...
  private extractTransactions(lines: string[]): ParsedRow[] {
    ...
    // General patterns
    const gtPattern =
      /^(\d{1,2}[-\s][A-Za-z]{3}[-\s](?:20\d{2}|\d{2}))\s+(.+?)\s+([-\d,]+\.\d{2})\s+([-\d,]+\.\d{2})(?:\s+[-\d,]+\.\d{2})?(?:\s*(?:CR|DR))?\s*$/i;
    const genericPattern =
      /^(\d{1,2}[/-]\d{1,2}[/-](?:20\d{2}|\d{2}))\s+(.+?)\s+([-\d,]+\.\d{2})\s*(CR|DR)?(?:\s+[-\d,]+\.\d{2})?(?:\s*(?:CR|DR))?\s*$/i;
    const dashPattern =
      /^(\d{1,2}[-\s][A-Za-z]{3}[-\s](?:20\d{2}|\d{2}))\s+(.+?)\s+([-\d,]+\.\d{2})(?:\s+[-\d,]+\.\d{2})?\s*$/i;

    // Access Bank Spaceless format
    // Single line: Date[Date]Description[Debit][Credit][Balance]
    const accessSingleLine =
      /^(\d{1,2}-[A-Za-z]{3}-(?:20\d{2}|\d{2}))(?:\d{1,2}-[A-Za-z]{3}-(?:20\d{2}|\d{2}))?(.*?)([\d,]+\.\d{2}|-)([\d,]+\.\d{2}|-)([\d,]+\.\d{2}|-)$/i;

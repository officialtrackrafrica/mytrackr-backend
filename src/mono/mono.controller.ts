...
export class MonoController {
  ...
  @ApiOperation({
    summary:
      'Get transactions for all linked accounts of the authenticated user',
  })
  @ApiQuery({
    name: 'paginate',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Aggregate transaction history for all accounts',
  })
  async getAllUserTransactions(
    @Req() req: any,
    @Query('paginate') paginate?: boolean,
    @Query('start') start?: string,
    @Query('end') end?: string,
    @Headers('x-real-time') realtime?: string,
  ) {
